...
namespace NodeEditorFramework
{
	...
	{
		public static string editorPath = "Assets/TextureWang/3rdParty/Node_Editor-master/Node_Editor/";
        public static string editorPathLoadSave = "Assets/TextureWang/";
		...
		private static void DrawSubCanvas (NodeCanvas nodeCanvas, NodeEditorState editorState)  
		{
            ...
            if (curEditorState.dragSelection)
            {
                //draw a box
                Vector2 startPos = curEditorState.dragStart;
                Vector2 endPos = curEditorState.dragStart + curEditorState.dragOffset;
                //startPos = NodeEditor.ScreenToCanvasSpace(startPos) + curEditorState.zoomPanAdjust;
                //endPos = NodeEditor.ScreenToCanvasSpace(endPos)+ curEditorState.zoomPanAdjust;
                //draw a line in screen space, our dragging is done in screen space not canvas space

                Vector2 topRight=new Vector2(endPos.x,startPos.y);
                Vector2 bottomLeft = new Vector2(startPos.x, endPos.y);
                RTEditorGUI.DrawLine(startPos, topRight, Color.white, null, 2);
                RTEditorGUI.DrawLine(topRight, endPos, Color.white, null, 2);
                RTEditorGUI.DrawLine(endPos, bottomLeft, Color.white, null, 2);
                RTEditorGUI.DrawLine(bottomLeft, startPos, Color.white, null, 2);
	    ...
	    public static void SelectNodes(Rect canvasRect)
	    {
            NodeCanvas canvas = curEditorState.canvas;
	        curEditorState.selectedNodes.Clear();
            foreach (var n in canvas.nodes)
	        {
	            if (n.rect.Overlaps(canvasRect))
	            {
                    curEditorState.selectedNodes.Add(n);
	            }
	        }
	    }
        ...
        /// Transforms screen space elements in the current editor into canvas space (Level of Nodes, ...) 
        /// </summary>
        public static Vector2 CanvasSpaceToScreen(Vector2 canvasPos)
        {
            return CanvasSpaceToScreen(curEditorState, canvasPos);
        }
        /// <summary>
        /// Transforms screen space elements in the specified editor into canvas space (Level of Nodes, ...) 
        /// </summary>
...
//                    Debug.Log(" worklist add "+node);
	    ...
	    static void AddOutputsToWorkList(Node n,List<Node> _workList )
	    {
            m_Done.Add(n,true);
            foreach (var x in n.Outputs)
            {
                foreach (var c in x.connections)
                {
                    ...
                    if (!m_Done.ContainsKey(c.body))
                    {
                        _workList.Add(c.body);
                        AddOutputsToWorkList(c.body, _workList);
                    }
                }
            }
        }
		...
		{
			node.ClearCalculation ();
			var workList = new List<Node> { node };
            m_Done=new Dictionary<Node, bool>();
            AddOutputsToWorkList(node, workList);
            StartCalculation (workList);
		}

	    private static Dictionary<Node, bool> m_Done;
        ...
        /// </summary>
        public static bool StartCalculation (List<Node> workList)
        {
            m_Depth++;
                checkInit (false);
			if (InitiationError)
			...
			{ // Runs until every node possible is calculated
				limitReached = true;
#if DEBUG_LOOPBASIC
                Debug.LogError(GetPad()+"Restart worklist " + roundCnt);
#endif

                
				for (int workCnt = 0; workCnt < workList.Count; workCnt++,count++) 
				{
#if DEBUG_LOOPBASIC
                    Debug.LogError(GetPad() + "iterante worklist " + workCnt+" node "+ workList[workCnt]);
#endif
                    if (ContinueCalculation (workList [workCnt],workList))
						limitReached = false;
				}
			    if (count > 10000 || limitReached)
			    {
#if DEBUG_LOOPBASIC
                    Debug.LogError(GetPad() + "finished worklist count:" + count+ " limitReached "+ limitReached);
#endif
			        m_Depth--;
                    return false;
			    }
...
#if DEBUG_LOOPBASIC
            Debug.LogError("finished worklist ");
#endif
            m_Depth--;
            return true;
        }
        ...
        /// </summary>
        public static bool ContinueCalculation (Node node, List<Node> workList)
		{
...
#if DEBUG_LOOPBASIC
            if(InputsCalculated || node.isInLoop())
                Debug.Log(GetPad() + "about to calculate " +node+" inputs calced "+InputsCalculated);
#endif
            ...
            if ((inputsCalculated || node.AllowRecursion) && (node.Calculate()))
		    {
#if DEBUG_LOOPBASIC
                Debug.Log(GetPad() + " " +calculationCount+" node "+node+" calc success");
#endif
		        // finished Calculating, continue with the children
		        node.calculated = true;
/*
		        int indexTimer = node.name.IndexOf("::"); 
                if (indexTimer!=-1 && indexTimer<4)
                    node.name = timer.ElapsedMilliseconds+"::"+node.name.Substring(indexTimer+2, node.name.Length- indexTimer-2) ;
                else
                    node.name =  timer.ElapsedMilliseconds+ "::"+ node.name;
*/
                calculationCount++;
		        workList.Remove(node);
		        if (node.ContinueCalculation && calculationCount < 10000)
		        {
		            for (int outCnt = 0; outCnt < node.Outputs.Count; outCnt++)
		            {
		                ...
		                if (!output.calculationBlockade)
		                {
		                    for (int conCnt = 0; conCnt < output.connections.Count; conCnt++)
		                        ContinueCalculation(output.connections[conCnt].body,workList);
		                }
		            }
		        }
		        else if (calculationCount >= 10000)
		        {
		            Debug.LogError(GetPad() +
                        "Stopped calculation because of suspected Recursion. Maximum calculation iteration is currently at 1000!");
		            return false;
		        }
		        return true; //we actually executed one so go through the list again to see if new ones Canvas execute
		    }
		    else
		    {
#if DEBUG_LOOPBASIC
                Debug.Log(GetPad() + " " + calculationCount + " node " + node + " fail or skipped inputsCalced: "+ InputsCalculated+" in loop "+ node.isInLoop()+" calc res "+res);
#endif
