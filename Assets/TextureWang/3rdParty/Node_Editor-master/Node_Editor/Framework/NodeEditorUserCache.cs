...
namespace NodeEditorFramework
{
	public class NodeEditorUserCache
	{
		...
		public void SetupCacheEvents (bool _loadCache) 
		{ 
			if (!useCache)
				return;
...
//			if (!nodeCanvas.livesInScene && UnityEditor.AssetDatabase.GetAssetPath (nodeCanvas) != lastSessionPath)
	//			throw new UnityException ("Cache system error: Current Canvas is not saved as the temporary cache! " +UnityEditor.AssetDatabase.GetAssetPath(nodeCanvas) +" != "+ lastSessionPath);
		...
		/// </summary>
		public void NewNodeCanvas (Type canvasType = null, int _texWidth = 1024, int _texHeight = 1024,ChannelType _pixelDepth=ChannelType.Float) 
		{
            ...
            nodeCanvas.m_DefaultTextureWidth = _texWidth;
            nodeCanvas.m_DefaultTextureHeight = _texHeight;
            nodeCanvas.m_DefaultChannelType = _pixelDepth;
